...
export async function POST(request: NextRequest) {
  try {
    ...
    const { department_name, contact_name, email, phone, password } = body

    if (!department_name || !contact_name || !email || !password) {
    ...
    if (clientError) {
      ...
      await adminClient.auth.admin.deleteUser(newUserId)
      return NextResponse.json({ error: '클라이언트 생성에 실패했습니다.' }, { status: 500 })
    }
